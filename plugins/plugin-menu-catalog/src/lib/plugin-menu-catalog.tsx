...
export interface MenuCatalogAPI {
  ...
  createProduct(data: {
    ...
    imageUrl?: string;
    categoryId: string;
    prices: { description: string; value: number }[];
    complements?: {
      group: string;
      title: string;
      description?: string;
  ...
  createComplement(data: {
    productId: string;
    group: string;
    title: string;
    description?: string;
    value?: number;
  }): Promise<ProductComplementDTO>;
  updateComplement(
    id: string,
    data: {
      group: string;
...
const styles = {
  ...
  empty: {
    color: 'var(--devxp-color-text-muted, #A0A0A0)',
...
const CategoryFormPage: React.FC<CategoryFormPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await menuCatalogAPI.updateCategory(categoryId, {
          name: name.trim(),
          description: description.trim() || undefined,
          imageUrl: imageUrl || undefined,
        });
      } else {
        await menuCatalogAPI.createCategory({
          name: name.trim(),
          description: description.trim() || undefined,
          imageUrl: imageUrl || undefined,
        });
  ...
  return (
    <div style={styles.page}>
      <Link href={`/plugins/${PLUGIN_ID}/categories`} style={styles.backLink}>
        ← Voltar
      </Link>
...
const ProductFormPage: React.FC<ProductFormPageProps> = ({
      ...
      menuCatalogAPI.getProductById(productId).then((prod) => {
        ...
        if (prod.complements.length > 0) {
          lastGroupRef.current =
            prod.complements[prod.complements.length - 1].group;
        }
      });
    }
  }, [productId, isEdit]);
  ...
  const addComplement = () => {
    setComplements((prev) => [
      ...prev,
      { group: lastGroupRef.current, title: '', description: '', value: '0' },
    ]);
  };
  ...
  const updateComplement = (
    index: number,
    field: 'group' | 'title' | 'description' | 'value',
    val: string,
  ) => {
    setComplements((prev) =>
  ...
  return (
    <div style={styles.page}>
      <Link href={`/plugins/${PLUGIN_ID}/products`} style={styles.backLink}>
        ← Voltar
      </Link>
      <Card title={isEdit ? 'Editar Produto' : 'Novo Produto'} padding="lg">
        <form onSubmit={handleSubmit}>
          <div style={styles.fieldGroup}>
            <label style={styles.label} htmlFor="prod-name">
              Nome
            </label>
            <input
              id="prod-name"
          ...
          <div style={styles.fieldGroup}>
            <label style={styles.label} htmlFor="prod-cat">
              Categoria
            </label>
            <select
              id="prod-cat"
          ...
          <div style={styles.fieldGroup}>
            <label style={styles.label} htmlFor="prod-desc">
              Descrição
            </label>
            <input
              id="prod-desc"
              ...
              <div key={index} style={styles.priceRow}>
                <div style={styles.priceField}>
                  ...
                  <input
                    ...
                    value={p.description}
                    onChange={(e) =>
                      updatePrice(index, 'description', e.target.value)
                    }
                ...
                <div style={styles.priceFieldSmall}>
                  ...
                  <input
                    ...
                    value={p.value}
                    onChange={(e) =>
                      updatePrice(index, 'value', e.target.value)
                    }
                    placeholder="0.00"
                    required
                  />
...
const CategoryListPage: React.FC = () => {
  ...
  return (
    <div style={styles.page}>
      <Link href={`/plugins/${PLUGIN_ID}`} style={styles.backLink}>
        ← Voltar
      </Link>
              ...
              <li key={c.id} style={styles.listItem}>
                  ...
                  {c.imageUrl ? (
                    ...
                    <div
                      style={{
                        ...
                        borderRadius: '4px',
                        backgroundColor:
                          'var(--devxp-color-surface-elevated, #2A2A2A)',
                        flexShrink: 0,
                      }}
                    />
                  )}
                ...
                <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
                  <IconButton
                    ...
                    variant="ghost"
                    size="sm"
                    href={`/plugins/${PLUGIN_ID}/categories/${c.id}`}
                    title="Editar"
                  />
                  <IconButton
                    icon="Trash2"
...
const ProductListPage: React.FC = () => {
  ...
  const displayPrice = (product: ProductDTO) => {
    ...
    if (product.prices.length === 1)
      return `R$ ${product.prices[0].value.toFixed(2)}`;
      ...
      <Card title="Produtos" padding="lg">
        ...
        >
          <button
            ...
            style={styles.pill(selectedCategory === 'all')}
          >
            Todos
          </button>
          {categories.map((cat) => (
            <button
              ...
              style={styles.pill(selectedCategory === cat.id)}
            >
              {cat.name}
            </button>
          ))}
              ...
              <li key={p.id} style={styles.listItem}>
                ...
                >
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    {p.complements && p.complements.length > 0 && (
                      ...
                      >
                        🔗 {p.complements.length} complemento
                        {p.complements.length > 1 ? 's' : ''}
                      </div>
                    )}
                  </div>
                </div>
                ...
                >
                  ...
                  <IconButton
                    ...
                    variant="ghost"
                    size="sm"
                    href={`/plugins/${PLUGIN_ID}/products/${p.id}`}
                    title="Editar"
                  />
                  <IconButton
                    icon="Trash2"
    ...
    <div style={styles.page}>
      <Card title="Cardápio" padding="lg">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <Link
            href={`/plugins/${PLUGIN_ID}/categories`}
            style={{
              ...styles.primaryBtn,
              textDecoration: 'none',
              textAlign: 'center',
          ...
          <Link
            href={`/plugins/${PLUGIN_ID}/products`}
            style={{
              ...styles.primaryBtn,
              textDecoration: 'none',
              textAlign: 'center',
...
const CategoryFormEditPage: React.FC<{ params?: { id?: string } }> = ({
  params,
}) => <CategoryFormPage categoryId={params?.id} />;
const ProductFormCreatePage: React.FC = () => <ProductFormPage />;
const ProductFormEditPage: React.FC<{ params?: { id?: string } }> = ({
  params,
